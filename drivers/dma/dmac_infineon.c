...
struct infineon_dmac_channel {
	...
	struct dma_block_config block_copies[CONFIG_INFINEON_MAX_XFERS];
...
static int infineon_dmac_config(const struct device *dev, uint32_t channel, struct dma_config *cfg)
{
	...
	if (block_count > CONFIG_INFINEON_MAX_XFERS) {
		LOG_ERR("Too many blocks: %u (max: %u)", block_count, CONFIG_INFINEON_MAX_XFERS);
		return -EINVAL;
	}
